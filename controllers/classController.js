...
//     // Send email to the trainer (optional)
//     await transporter.sendMail({
//       from: process.env.ADMIN,
//       to: trainer.email,
//       subject: `New Class Assigned: ${name}`,
//       text: `You've been assigned to teach "${name}" on ${new Date(dateTime).toLocaleString()} at ${location}.`
//     });
...
//         text: `Hey ${
//           member.name
//         },\n\nA new class "${name}" has been scheduled on ${new Date(
//           dateTime
//         ).toLocaleString()} at ${location} with trainer ${
//           trainer.name
//         }. Book your spot now!\n\n- Gym Management`,
//       })
//     );
...
const createClass = async (req, res) => {
  ...
  try {
    ...
    // ✅ Now handle emails in the background
    ...
    transporter
      .sendMail({
        ...
        to: trainer.email,
        subject: `New Class Assigned: ${name}`,
        text: `You've been assigned to teach "${name}" on ${new Date(
          dateTime
...
const getAllClasses = async (req, res) => {
  try {
    ...
    const query = isAdmin
      ? {}
      : { dateTime: { $gte: currentDate }, status: "scheduled" };
...
const memberBookClass = async (req, res) => {
  try {
    ...
    if (!cls || cls.status !== "scheduled")
      return res
        .status(404)
        .json({ success: false, message: "Class not found or not available" });
    if (cls.bookings.length >= cls.maxCapacity)
      return res
        .status(400)
        .json({ success: false, message: "Class is fully booked" });
    if (cls.bookings.includes(userId))
      return res
        .status(400)
        .json({ success: false, message: "You’ve already booked this class" });
    if (new Date(cls.dateTime) < new Date())
      return res
        .status(400)
        .json({ success: false, message: "Class has already started" });
    ...
    await sendEmail(
      ...
      `Your booking for ${cls.name} on ${new Date(
        cls.dateTime
      ).toLocaleString()} with ${trainer.name} at ${
        cls.location
      } is confirmed. Get ready for the class!`
    );
    res
      .status(200)
      .json({ success: true, message: "Class booked successfully" });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
};
...
const classCancelByMember = async (req, res) => {
  try {
    ...
    if (!cls.bookings.includes(userId))
      return res
        .status(400)
        .json({ success: false, message: "You haven’t booked this class" });
    if (new Date(cls.dateTime) < new Date())
      return res
        .status(400)
        .json({ success: false, message: "Class has already started" });
