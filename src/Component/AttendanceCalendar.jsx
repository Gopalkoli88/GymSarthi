  ...
  return (
    <div className="w-full max-w-md p-3 mx-auto border border-blue-600 shadow-lg sm:max-w-2xl md:max-w-4xl sm:p-6 md:p-8 bg-gradient-to-br from-gray-800 via-gray-900 to-black rounded-2xl">
      ...
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <button onClick={handlePrevMonth} className="p-2 transition rounded-full hover:bg-blue-800">
          <ChevronLeft className="w-5 h-5 text-white sm:w-6 sm:h-6" />
        </button>
        <h2 className="text-lg font-bold text-white sm:text-xl">
          {currentDate.toLocaleString("default", { month: "long", year: "numeric" })}
        </h2>
        <button onClick={handleNextMonth} className="p-2 transition rounded-full hover:bg-blue-800">
      ...
      <div className="grid grid-cols-7 gap-1 text-xs font-semibold text-center text-blue-400 sm:text-sm sm:gap-2">
        {daysOfWeek.map((day) => (
          <div key={day} className="p-1 bg-gray-700 rounded-md shadow-md sm:p-2">
            {day}
          </div>
        ))}
      </div>
          ...
          return (
            <div
              ...
              className={`flex items-center justify-center 
                ...
                ${attended ? "bg-gradient-to-r from-green-500 to-green-600 text-white shadow-lg scale-105" : "bg-gray-800 text-gray-300"} 
                ${isToday ? (attended ? "bg-gradient-to-r from-green-500 to-green-600 text-white scale-110" : "bg-gradient-to-r from-blue-600 to-blue-800 text-white scale-110 shadow-lg") : "hover:bg-gray-700 hover:text-white"}`}
            >
              {day}
            </div>
          );
      ...
      <div className="mt-4 text-center sm:mt-5">
        <p className="p-2 text-sm font-bold text-white bg-blue-600 rounded-lg shadow-md sm:p-3 sm:text-lg hover:bg-blue-700 hover:scale-105">
          Total Attendances: <span className="text-green-300">{totalAttendances}</span>
        </p>
      </div>
