...
//   const { userFeedbacks } = useSelector((state) => state.feedback);
//   useEffect(() => {
//     if (!autoplay) return;
//     const timer = setInterval(() => {
//       setCurrentSlide((prev) => (prev + 1) % userFeedbacks.length);
//     }, 5000);
...
function Reviewss() {
  ...
  useEffect(() => {
    if (!autoplay || userFeedbacks.length === 0) return;
    const timer = setInterval(() => {
      setCurrentSlide((prev) => (prev + 1) % userFeedbacks.length);
    }, 5000);
    return () => clearInterval(timer);
  }, [autoplay, userFeedbacks]);

  if (userFeedbacks.length === 0) return null;

  const current = userFeedbacks[currentSlide];
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="grid md:grid-cols-[300px,1fr] gap-8 items-center">
                ...
                <div className="relative">
                  <motion.div
                    animate={{ rotate: [-3, 0, -3], scale: [1, 1.02, 1] }}
                    transition={{
                      duration: 5,
                      repeat: Infinity,
                      repeatType: "reverse",
