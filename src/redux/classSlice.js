...
export const createClass = createAsyncThunk('class/create', async (classData, { getState, rejectWithValue }) => {
  const { token } = getState().user;
  try {
    const response = await api.post('/class/admin/classes', classData, { headers: { Authorization: `Bearer ${token}` } });
...
export const fetchClasses = createAsyncThunk('class/fetch', async (_, { getState, rejectWithValue }) => {
  const { token } = getState().user;
  try {
    const response = await api.get('/class/classes', { headers: { Authorization: `Bearer ${token}` } });
...
export const fetchTrainerSchedule = createAsyncThunk('class/trainerSchedule', async (_, { getState, rejectWithValue }) => {
  const { token } = getState().user;
  try {
    const response = await api.get('/class/trainer/schedule', { headers: { Authorization: `Bearer ${token}` } });
    return response.data.classes;
  } catch (error) {
...
export const cancelBooking = createAsyncThunk('class/cancel', async (classId, { getState, rejectWithValue }) => {
    const { token } = getState().user;
    try {
      const response = await api.delete(`/class/classes/${classId}/book`, { headers: { Authorization: `Bearer ${token}` } });
